...
# 🔥 V221: CÓDIGO LIMPIO, SINCRONÍA TOTAL ANTI-GAP Y CERO CORTES 🔥
if st.session_state.get('app_version') != 'V221':
    st.session_state.clear()
    st.session_state['app_version'] = 'V221'
...
def simular_crecimiento_exponencial_ia_core(h_arr, l_arr, c_arr, o_arr, atr_arr, rsi_arr, z_arr, adx_arr, 
    ...
    for i in range(len(h_arr)):
        ...
        if en_pos:
            ...
            # 🔥 EVALUACIÓN INTRABAR (Sin restricción de barras, puede salir en la misma vela de entrada)
            if l_arr[i] <= sl_p:
                # GAP CHECK: Si abre por debajo del SL, tomamos el precio de apertura
                ...
                # GAP CHECK: Si abre por encima del TP, tomamos el precio de apertura
            ...
            # 🔥 SALIDA DINÁMICA (Si no tocó TP ni SL)
            if not cierra:
                score = (rsi_arr[i] * w_rsi) + (z_arr[i] * w_z) + (adx_arr[i] * w_adx)
                if s_c[i] or (score < th_sell):
                    # Se vende a la apertura de la vela de mañana, sufriendo slippage real
                    if i + 1 < len(o_arr):
...
# ==========================================
st.sidebar.markdown("<h2 style='text-align: center; color: cyan;'>🧬 GENESIS LAB V221</h2>", unsafe_allow_html=True)
...
def generar_reporte_universal(cap_ini, com_pct):
    res_str = f"📋 **REPORTE GENESIS LAB V221.0**\n\n"
...
# ==========================================
@st.cache_data(ttl=3600, show_spinner="📡 Sincronizando Línea Temporal con TradingView (V221)...")
...
"""
    ...
    # 🔥 V221: BLOQUE PINE SCRIPT INSTITUCIONAL BLINDADO Y SIN ERRORES 🔥
    ps_exec = """
var float locked_atr = na
var float tp_price = na
var float sl_price = na

// 1. DETECCIÓN DE ENTRADA REAL
