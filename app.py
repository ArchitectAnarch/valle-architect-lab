...
rsi_cross_up = rsi > nz(rsi[1], 50) and rsi[1] <= nz(rsi[1], 50)
rsi_cross_dn = rsi < nz(rsi[1], 50) and rsi[1] >= nz(rsi[1], 50)
...
"""
        ...
        ps_indicators += """
pl30 = nz(ta.pivotlow(low, 30, 3)), ph30 = nz(ta.pivothigh(high, 30, 3))
pl100 = nz(ta.pivotlow(low, 100, 5)), ph100 = nz(ta.pivothigh(high, 100, 5))
pl300 = nz(ta.pivotlow(low, 300, 5)), ph300 = nz(ta.pivothigh(high, 300, 5))
a_tsup = math.max(pl30, pl100, pl300), a_tres = math.min(ph30, ph100, ph300)
"""
    ps_indicators += """
...
div_bull = nz(low[1] < low[5]) and nz(rsi[1] > rsi[5]) and (rsi < 35)
div_bear = nz(high[1] > high[5]) and nz(rsi[1] < rsi[5]) and (rsi > 65)
...
if st.session_state.get('run_global', False):
    if len(st.session_state['global_queue']) > 0:
        ...
        # Refresca la página automáticamente para procesar el siguiente sin colgar el servidor
        st.rerun()
    else:
...
for tab_obj, tab_name in zip(ui_tabs, tab_names):
    with tab_obj:
        ...
        with st.expander("📝 PINE SCRIPT GENERATOR", expanded=False):
            st.info("Traducción Matemática Idéntica a TradingView. Si gana aquí, gana allá.")
