...
# 🔥 AUTO-PURGA DE CACHÉ SUCIA Y REINICIO 🔥
if st.session_state.get('app_version') != 'V135':
    st.session_state.clear()
    st.session_state['app_version'] = 'V135'

# ==========================================
# 🧠 1. UTILIDADES NUMPY Y RÉPLICAS TV
# ==========================================
...
# ==========================================
st.sidebar.markdown("<h2 style='text-align: center; color: cyan;'>🚀 OMNI-FORGE V135.0</h2>", unsafe_allow_html=True)
...
def generar_reporte_universal(cap_ini, com_pct):
    res_str = f"📋 **REPORTE OMNI-FORGE V135.0**\n\n"
...
if st.sidebar.button("📊 GENERAR REPORTE UNIVERSAL", use_container_width=True, key="btn_univ_report"):
    st.sidebar.text_area("Block Note Universal:", value=generar_reporte_universal(capital_inicial, comision_pct), height=400)

@st.cache_data(ttl=3600, show_spinner="📡 Extrayendo y Calculando Geometría Fractal (V135)...")
...
# ==========================================
# 🛑 5. EXTRACCIÓN DE ARRAYS (Para Numba)
# ==========================================
...
if not df_global.empty: 
    ...
    # ==========================================
    # 🧠 6. FUNCIONES MAESTRAS (Scope Seguro y Velocidad Dual)
    # ==========================================
    ...
    # 🔥 NUMBA SCALAR (Para bots de un solo TP/SL: 100x más rápido)
    @njit(fastmath=True)
    ...
    # 🔥 NUMBA ARRAY (Para Genesis/Quadrix que mutan TP/SL por cuadrante)
    @njit(fastmath=True)
    def simular_crecimiento_exponencial_array(h_arr, l_arr, c_arr, o_arr, b_c, s_c, t_arr, sl_arr, cap_ini, com_pct, reinvest_pct):
        cap_act = cap_ini; divs = 0.0; en_pos = False; p_ent = 0.0
        tp_act = 0.0; sl_act = 0.0; pos_size = 0.0; invest_amt = 0.0; g_profit = 0.0; g_loss = 0.0; num_trades = 0; max_dd = 0.0; peak = cap_ini
        for i in range(len(h_arr)):
            if en_pos:
    ...
    # 🚀 MOTOR DE BÚSQUEDA DUAL-LOOP (Cero sobrecarga de RAM) 🚀
    def optimizar_ia_tracker(s_id, cap_ini, com_pct, reinv_q, target_ado, dias_reales, buy_hold_money, epochs=1, cur_fit=-float('inf')):
        ...
        # Bloques de ADN (Solo 50 cálculos de Numpy para toda la simulación)
        chunks = min(50, epochs * 10) 
        chunk_size = (3000 * epochs) // chunks 
        ...
        if is_multi: f_tp_arr = np.zeros(n_len, dtype=np.float64); f_sl_arr = np.zeros(n_len, dtype=np.float64)

        for c in range(chunks):
            if st.session_state.get('abort_opt', False): break
            
            # 1. Bucle Externo: Extracción de ADN
            ...
            # Pre-ensamblaje de señales base para no recalcular en el bucle interno
            if s_id in ["ROCKET_ULTRA", "ROCKET_COMMANDER"]:
                ...
                macro_mask = a_mb if dna_macro == "Bull Only (Precio > EMA 200)" else (~a_mb if "Bear" in dna_macro else np.ones(n_len, dtype=bool))
                vol_mask = (a_adx >= r_adx) if dna_vol == "Trend (ADX Alto)" else ((a_adx < r_adx) if "Range" in dna_vol else np.ones(n_len, dtype=bool))
            ...
            # 2. Bucle Interno (C++ JIT PURO) - Ejecución de Miles de TPs/SLs en Milisegundos
            for _ in range(chunk_size): 
                if is_multi:
                    dna_tp = [random.uniform(tp_min, tp_max) for _ in range(4)]; dna_sl = [random.uniform(0.5, 20.0) for _ in range(4)]
                    for idx in range(4): f_tp_arr[masks[idx]] = dna_tp[idx]; f_sl_arr[masks[idx]] = dna_sl[idx]
                    ...
                    net, pf, nt, mdd = simular_crecimiento_exponencial_scalar(a_h, a_l, a_c, a_o, f_buy, f_sell, rtp, rsl, float(cap_ini), float(com_pct), float(reinv_q))
            ...
            # Actualización Segura y Rápida
            if c % max(1, chunks // 4) == 0 or c == chunks - 1:
                ...
                <div class="loader-container">
                    ...
                    <div class="prog-text">OMNI-FORGE V135: {s_id}</div>
    ...
    def simular_visual_wrapper(s_id, cap_ini, com_pct):
        vault = st.session_state[f'champion_{s_id}']
        ...
        if s_id in ["ROCKET_ULTRA", "ROCKET_COMMANDER"]:
            ...
            for idx_q in range(1, 5):
                ...
                f_tp[mask] = vault[f'r{idx_q}_tp']; f_sl[mask] = vault[f'r{idx_q}_sl']
        else: 
            b_k = f"{s_id.split('_')[0].capitalize()}_Buy" if s_id not in ["TARGET_LOCK", "NEON_SQUEEZE", "PINK_CLIMAX", "PING_PONG"] else "Lock_Buy" if s_id == "TARGET_LOCK" else "Squeeze_Buy" if s_id == "NEON_SQUEEZE" else "Climax_Buy" if s_id == "PINK_CLIMAX" else "Ping_Buy"
            s_k = f"{s_id.split('_')[0].capitalize()}_Sell" if s_id not in ["TARGET_LOCK", "NEON_SQUEEZE", "PINK_CLIMAX", "PING_PONG"] else "Lock_Sell" if s_id == "TARGET_LOCK" else "Squeeze_Sell" if s_id == "NEON_SQUEEZE" else "Climax_Sell" if s_id == "PINK_CLIMAX" else "Ping_Sell"
            f_buy[:] = s_dict.get(b_k, np.zeros(n_len, dtype=bool)); f_sell[:] = s_dict.get(s_k, np.zeros(n_len, dtype=bool))
    ...
    for tab_obj, tab_name in zip(ui_tabs, tab_names):
        with tab_obj:
            ...
            df_strat, eq_curve, t_log, total_comms = simular_visual_wrapper(s_id, capital_inicial, comision_pct)
