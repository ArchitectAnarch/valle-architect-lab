...
st.set_page_config(page_title="ROCKET PROTOCOL | Genesis Lab V355", layout="wide", initial_sidebar_state="expanded")
ph_holograma = st.empty()

# 🔥 V355: THE OMNI-BRAIN (TRINITY + PINK WHALE + ASYMMETRIC EXECUTION) 🔥
APP_VERSION = 'V355'
...
# ==========================================
todas_las_armas_b = ['Q_Pink_Whale_Buy', 'Q_Lock_Bounce', 'Q_Lock_Break', 'Q_Neon_Up', 'Q_Defcon_Buy', 'Q_Therm_Bounce', 'Q_Therm_Vacuum']
todas_las_armas_s = ['Q_Lock_Reject', 'Q_Lock_Breakd', 'Q_Neon_Dn', 'Q_Defcon_Sell', 'Q_Therm_Wall_Sell', 'Q_Therm_Panic_Sell']

pine_map = {
    'Q_Pink_Whale_Buy': 'r_Pink_Whale_Buy', 'Q_Lock_Bounce': 'r_Lock_Bounce', 'Q_Lock_Break': 'r_Lock_Break', 
    'Q_Neon_Up': 'r_Neon_Up', 'Q_Defcon_Buy': 'r_Defcon_Buy', 'Q_Therm_Bounce': 'r_Therm_Bounce', 'Q_Therm_Vacuum': 'r_Therm_Vacuum', 
    'Q_Lock_Reject': 'r_Lock_Reject', 'Q_Lock_Breakd': 'r_Lock_Breakd', 'Q_Neon_Dn': 'r_Neon_Dn', 
    'Q_Defcon_Sell': 'r_Defcon_Sell', 'Q_Therm_Wall_Sell': 'r_Therm_Wall_Sell', 'Q_Therm_Panic_Sell': 'r_Therm_Panic_Sell'
}

if 'ai_algos' not in st.session_state or len(st.session_state['ai_algos']) == 0: 
    st.session_state['ai_algos'] = [f"TRINITY_{random.randint(100, 999)}"]
...
def get_default_dna():
    return {
        ...
        'net': 0.0, 'winrate': 0.0, 'pf': 0.0, 'nt': 0,
...
# ==========================================
st.sidebar.markdown("<h2 style='text-align: center; color: cyan;'>🧬 TRINITY LAB V355</h2>", unsafe_allow_html=True)
...
with st.sidebar.expander("🤖 INTELIGENCIA Y FORJA", expanded=False):
    ...
    if st.button("🤖 CREAR NUEVO MUTANTE IA", type="secondary", use_container_width=True, key="btn_mutant"):
        new_id = f"TRINITY_{int(time.time())}_{random.randint(10, 99)}"
    ...
    if st.button("🌌 CREAR MUTANTE PROFUNDO", type="secondary", use_container_width=True, key="btn_mutant_deep"):
        new_id = f"DEEP_TRINITY_{int(time.time())}_{random.randint(10, 99)}"
        if new_id not in st.session_state['ai_algos']:
...
if 'data_params' not in st.session_state:
    st.info("👈 Por favor, configura los datos en el menú lateral y haz clic en **'📥 DESCARGAR MATRIX DE DATOS'** para iniciar la Forja.")
    st.stop()
...
# ==========================================
@st.cache_data(ttl=3600, show_spinner="📡 Sincronizando Línea Temporal con Servidores (V355)...")
...
def calcular_señales_numpy(hitbox, therm_w, adx_th, whale_f):
    ...
    base_mask = retro_peak | k_break_up | (is_grav_sup & a_rcu)
    buy_score = np.where(base_mask & retro_peak, 50.0, np.where(base_mask & ~retro_peak, 30.0, buy_score))
    buy_score += np.where(is_grav_sup, 25.0, 0.0); buy_score += np.where(k_break_up & ~retro_peak, 15.0, 0.0); buy_score += np.where(a_zscore < -2.0, 15.0, 0.0)
...
def optimizar_ia_tracker(s_id, cap_ini, com_pct, invest_pct, target_ado, dias_reales, buy_hold_money, epochs=1, cur_net=-float('inf'), cur_fit=-float('inf'), deep_info=None, greed_factor=0.8):
    ...
    for c in range(chunks):
        ...
        for _ in range(chunk_size): 
            ...
            dna_b_op = random.choice(['|']) # Trinity prefiere OR
        ...
        if deep_info:
            ...
            title = f"🌌 DEEP FORGE V355: {s_id}"; subtitle = f"Épocas: {current_epoch_val:,} / {deep_info['total']:,} ({macro_pct}%)<br>⏱️ Tiempo: {time_str}"; color = "#FF00FF"
        else:
            pct_done = int(((c + 1) / chunks) * 100); combos = (c + 1) * chunk_size
            title = f"TRINITY LAB V355: {s_id}"; subtitle = f"Progreso: {pct_done}% | ADN Probados: {combos:,}<br>⏱️ Tiempo Ejecución: {time_str}"; color = "#00FFFF"
...
def generar_pine_script(s_id, vault, sym, tf, buy_pct, sell_pct, com_pct, start_date_obj, is_calib=False):
    ...
    ps_base = f"""//@version=5
strategy("{s_id} TRINITY V355 - {sym} [{tf}]", overlay=true, initial_capital=1000, default_qty_type=strategy.percent_of_equity, default_qty_value={buy_pct}, commission_type=strategy.commission.percent, commission_value={com_pct*100}, process_orders_on_close=true)
...
if just_entered
    tp_price := strategy.position_avg_price * 1.002
    sl_price := strategy.position_avg_price * 0.998

if strategy.position_size > 0
    if high >= tp_price or low <= sl_price
        strategy.close("In", comment="TP/SL", alert_message=wt_exit_long)
        tp_price := na
        sl_price := na
...
r_Therm_Bounce = (floor_w >= therm_wall) and rsi_cross_up and not is_hard_wall
r_Therm_Vacuum = (ceil_w <= 3) and neon_up and not is_abyss
r_Therm_Wall_Sell = (ceil_w >= therm_wall) and rsi_cross_dn
r_Therm_Panic_Sell = is_abyss and vela_roja
...
r_Lock_Bounce = is_grav_sup and (low <= a_tsup + tol) and (close > a_tsup) and vela_verde
r_Lock_Break = is_grav_res and cross_up_res and vela_verde
r_Lock_Reject = is_grav_res and (high >= a_tres - tol) and (close < a_tres) and vela_roja
r_Lock_Breakd = is_grav_sup and cross_dn_sup and vela_roja
...
buy_score = 0.0
base_mask = retro_peak or k_break_up or (is_grav_sup and rsi_cross_up)
...
r_Pink_Whale_Buy = is_magenta and is_whale_icon
"""
...
# ==========================================
st.title("🛡️ GENESIS LAB V355 - The Omni-Brain")
...
if len(tab_names) > 0:
    ...
    if is_calib_mode:
        ...
        with st.expander("🧬 VER ADN DEL MUTANTE Y ARMAS TÁCTICAS", expanded=True):
            st.markdown(f"**🟢 Módulos de Compra Activos:** {', '.join(vault.get('b_team', []))}")
            st.markdown(f"**🔴 Módulos de Venta Activos:** {', '.join(vault.get('s_team', []))}")
            st.markdown(f"**🎯 Parámetros:** TP: `{vault.get('tp_pct',0):.2f}%` | SL: `{vault.get('sl_pct',0):.2f}%`")
            st.markdown(f"**📡 Sensibilidad:** Hitbox: `{vault.get('hitbox',0):.1f}%` | Ballena: `{vault.get('whale_f',0):.1f}x` | Termómetro: `{vault.get('therm_w',0)}`")
