...
if st.session_state.get('app_version') != 'V132':
    st.session_state.clear()
    st.session_state['app_version'] = 'V132'
...
doctrinas = {
    ...
    "ROCKET_COMMANDER": "El Almirante (V60.2). Cruza el Radar de Gravedad con Osciladores WaveTrend. Detecta anomalías 'Magenta'.",
...
# ==========================================
st.sidebar.markdown("<h2 style='text-align: center; color: cyan;'>🚀 OMNI-FORGE V132.0</h2>", unsafe_allow_html=True)
...
# 📊 BLOCK NOTE UNIVERSAL Y GITHUB
def generar_reporte_universal(cap_ini, com_pct):
    res_str = f"📋 **REPORTE OMNI-FORGE V132.0**\n\n"
...
@st.cache_data(ttl=3600, show_spinner="📡 Construyendo Geometría Fractal & WaveTrend (V132)...")
...
if st.session_state.get('run_global', False) and not df_global.empty:
    ...
    st.sidebar.success("✅ ¡Forja Evolutiva Global Completada!"); time.sleep(1); st.rerun()
...
if st.session_state.get('run_ai_mutant', False) and not df_global.empty:
    ...
    st.sidebar.success(f"🤖 ¡Mutante {mutant_id} Creado y Forjado!"); time.sleep(1); st.rerun()
...
with st.expander("🏆 SALÓN DE LA FAMA (Ordenado por Rentabilidad Neta)", expanded=False):
    ...
    for s in estrategias:
        ...
        if fit != -float('inf'):
            net_val = v.get('net', 0)
            leaderboard_data.append({"Estrategia": s, "Neto_Num": net_val, "Rentabilidad Neta": f"${net_val:,.2f} ({net_val/capital_inicial*100:.2f}%)", "WinRate": f"{v.get('winrate', 0):.1f}%", "Puntaje IA": f"{fit:,.0f}"})
    if leaderboard_data:
...
def calcular_señales_numpy(s_id, hitbox, therm_w, adx_th, whale_f):
    ...
    # Simulación de Therm Walls para evitar RAM Exhaustion
    is_hard_wall = a_dres < 1.0 
    is_abyss = a_dsup > 5.0
    cond_therm_buy_bounce = (a_dsup < 1.5) & a_rcu & ~is_hard_wall
    cond_therm_buy_vacuum = ~is_hard_wall & neon_up & ~is_abyss
    cond_therm_sell_wall = is_hard_wall & a_rcd
    ...
    matrix_active = is_grav_sup | is_hard_wall
...
def simular_crecimiento_exponencial(h_arr, l_arr, c_arr, o_arr, b_c, s_c, t_arr, sl_arr, cap_ini, com_pct, reinvest_pct):
    cap_act = cap_ini; divs = 0.0; en_pos = False
    p_ent = 0.0; tp_act = 0.0; sl_act = 0.0
    pos_size = 0.0; invest_amt = 0.0
    g_profit = 0.0; g_loss = 0.0; num_trades = 0; max_dd = 0.0; peak = cap_ini
    ...
    for c in range(chunks):
        ...
        if c % ui_update_interval == 0 or c == chunks - 1:
            ...
            <div class="loader-container">
                ...
                <div class="prog-text">OMNI-FORGE V132: {s_id}</div>
...
"""
    if s_id == "ROCKET_COMMANDER":
        return f"""//@version=5
// ROCKET COMMANDER V60.2 ORIGINAL (Variables Optimizadas por IA)
...
// TP Base: {vault.get('tp', 0.0)}% | SL Base: {vault.get('sl', 0.0)}%"""
        ...
        return f"""//@version=5
// ROCKET PROTOCOL ULTRA [V55.0] ORIGINAL (Variables Optimizadas por IA)
// Copie su script original y reemplace los valores por defecto con estos:
// Hitbox: {vault['hitbox']} | Therm_Wall: {vault['therm_w']} | ADX: {vault['adx_th']} | Whale Factor: {vault['whale_f']}
// TP Base: {vault.get('tp', 0.0)}% | SL Base: {vault.get('sl', 0.0)}%"""
        ...
        return f"""//@version=5
// JUGGERNAUT V356 ORIGINAL (Variables Optimizadas por IA)
// Copie su script original y reemplace los valores por defecto con estos:
// Hitbox: {vault['hitbox']} | Therm_Wall: {vault['therm_w']} | ADX: {vault['adx_th']} | Whale Factor: {vault['whale_f']}
