...
@njit(fastmath=True)
def simular_crecimiento_exponencial_ia_core(h_arr, l_arr, c_arr, o_arr, atr_arr, rsi_arr, z_arr, adx_arr, b_c, s_c, w_rsi, w_z, w_adx, th_buy, th_sell, atr_tp_mult, atr_sl_mult, cap_ini, com_pct, invest_pct, slippage_pct, m_mask, v_mask):
    cap_act = cap_ini; en_pos = False; p_ent = 0.0; pos_size = 0.0; invest_amt = 0.0; g_profit = 0.0; g_loss = 0.0; num_trades = 0; max_dd = 0.0; peak = cap_ini
    ...
    for i in range(len(h_arr)):
        ...
        if en_pos:
            bars_in_trade += 1; cierra = False
            if bars_in_trade > 1: 
                if l_arr[i] <= sl_p:
                    exec_p = sl_p if o_arr[i] > sl_p else o_arr[i]
                    exec_p = exec_p * slip_out
                    ret = (exec_p - p_ent) / p_ent; gross = pos_size * (1.0 + ret); net = gross - (gross * com_pct); profit = net - invest_amt
                    cap_act += profit; g_loss += abs(profit); num_trades += 1; en_pos = False; cierra = True
                    ...
                    exec_p = exec_p * slip_out
                    ret = (exec_p - p_ent) / p_ent; gross = pos_size * (1.0 + ret); net = gross - (gross * com_pct); profit = net - invest_amt
                    cap_act += profit; g_profit += profit; num_trades += 1; en_pos = False; cierra = True
                    if profit > 0: wins += 1
                    if cap_act > peak: peak = cap_act
                    if peak > 0: dd = (peak - cap_act) / peak * 100.0; max_dd = max(max_dd, dd)
            if not cierra:
                ...
                if s_c[i] or (score < th_sell):
                    ...
                    cap_act += profit
                    if profit > 0: g_profit += profit; wins += 1
                    else: g_loss += abs(profit)
                    num_trades += 1; en_pos = False; just_closed_dyn = True
            if cap_act <= 0: break
        if not en_pos and not just_closed_dyn and i+1 < len(h_arr):
            ...
            if (b_c[i] or (score > th_buy)) and m_mask[i] and v_mask[i]:
                ...
                tp_p = p_ent + (current_atr * atr_tp_mult); sl_p = p_ent - (current_atr * atr_sl_mult)
...
def simular_visual(df_sim, cap_ini, invest_pct, com_pct, slippage_pct=0.0):
    ...
    atr_arr = df_sim['ATR'].values; buy_arr, sell_arr = df_sim['Signal_Buy'].values, df_sim['Signal_Sell'].values
    tp_arr, sl_arr = df_sim['Active_TP'].values, df_sim['Active_SL'].values; f_arr = df_sim.index
    en_pos, p_ent, tp_p, sl_p, cap_act, pos_size, invest_amt, total_comms = False, 0.0, 0.0, 0.0, cap_ini, 0.0, 0.0, 0.0
    bars_in_trade = 0; slip_in = 1.0 + (slippage_pct/100.0); slip_out = 1.0 - (slippage_pct/100.0)
    for i in range(n):
        cierra = False; just_closed_dyn = False
        if en_pos:
            bars_in_trade += 1
            if bars_in_trade > 1:
                if l_arr[i] <= sl_p:
        ...
        if not en_pos and not just_closed_dyn and buy_arr[i] and i+1 < n and cap_act > 0:
            ...
            p_ent = o_arr[i+1] * slip_in; tp_act = atr_arr[i] * float(df_sim['Active_TP'].values[i]); sl_act = atr_arr[i] * float(df_sim['Active_SL'].values[i])
            tp_p = p_ent + tp_act; sl_p = p_ent - sl_act; en_pos = True; bars_in_trade = 0
...
def simular_monte_carlo(trades_list, cap_ini, num_simulations=1000):
    ...
    rets_arr = np.array(rets); n_trades = len(rets_arr); mc_curves = np.zeros((num_simulations, n_trades + 1)); mc_curves[:, 0] = cap_ini; ruined_count = 0
    for i in range(num_simulations):
        np.random.shuffle(rets_arr)
        for j in range(n_trades):
            mc_curves[i, j+1] = mc_curves[i, j] + rets_arr[j]
            if mc_curves[i, j+1] <= 0: mc_curves[i, j+1:] = 0; ruined_count += 1; break
    return mc_curves, (ruined_count / num_simulations) * 100.0
...
if st.sidebar.button("🔄 Purgar Memoria & Sincronizar", use_container_width=True, key="btn_purge"): 
    st.cache_data.clear(); keys_to_keep = ['app_version', 'ai_algos']
...
if st.sidebar.button("🛑 ABORTAR RUN GLOBAL (Y MOSTRAR CAMPEÓN)", use_container_width=True, key="btn_abort"):
    st.session_state['abort_opt'] = True; st.session_state['global_queue'] = []; st.session_state['run_global'] = False; st.session_state['deep_opt_state'] = {}; st.rerun()
...
intervalo_sel = st.sidebar.selectbox("Temporalidad", list(intervalos.keys()), index=2); iv_download = intervalos[intervalo_sel]
hoy = datetime.today().date(); is_micro = iv_download in ["1m", "5m", "15m", "30m"]
...
if st.sidebar.button("🌌 CREAR MUTANTE PROFUNDO", type="secondary", use_container_width=True, key="btn_mutant_deep"):
    new_id = f"AI_DEEP_{random.randint(100, 999)}"; st.session_state['ai_algos'].append(new_id); estrategias.append(new_id); get_safe_vault(new_id); st.session_state['abort_opt'] = False; st.session_state['deep_opt_state'] = {'s_id': new_id, 'target_epochs': deep_epochs_target, 'current_epoch': 0, 'paused': False, 'start_time': time.time()}; st.rerun()
...
def generar_reporte_universal(cap_ini, com_pct):
    res_str = f"📋 **REPORTE GENESIS LAB V220.0**\n\n⏱️ Temporalidad: {intervalo_sel} | 📊 Ticker: {ticker}\n\n"
    for s_id in estrategias:
        v = get_safe_vault(s_id); opt_icon = "✅" if st.session_state.get(f'opt_status_{s_id}', False) else "➖"
...
def cargar_matriz(exchange_id, sym, start, end, iv_down, offset, is_micro):
    try:
        ex_class = getattr(ccxt, exchange_id)({'enableRateLimit': True}); warmup_days = 40 if is_micro else 150
        warmup_start = start - timedelta(days=warmup_days); start_ts = int(datetime.combine(warmup_start, datetime.min.time()).timestamp() * 1000); end_ts = int((datetime.combine(end, datetime.min.time()) + timedelta(days=1)).timestamp() * 1000)
        ...
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms'); df.set_index('timestamp', inplace=True); df.index = df.index + timedelta(hours=offset); df = df[~df.index.duplicated(keep='first')]
        if len(df) < 50: return pd.DataFrame(), f"❌ Solo {len(df)} velas."
        df['EMA_200'] = df['Close'].ewm(span=200, min_periods=1, adjust=False).mean(); df['EMA_50'] = df['Close'].ewm(span=50, min_periods=1, adjust=False).mean()
        df['Vol_MA_20'] = df['Volume'].rolling(window=20, min_periods=1).mean(); df['Vol_MA_100'] = df['Volume'].rolling(window=100, min_periods=1).mean()
        df['RVol'] = df['Volume'] / df['Vol_MA_100'].replace(0, 1); df['High_Vol'] = df['Volume'] > df['Vol_MA_20']
        df['ATR'] = ta.atr(df['High'], df['Low'], df['Close'], length=14).fillna(df['High']-df['Low']).replace(0, 0.001)
        df['RSI'] = ta.rsi(df['Close'], length=14).fillna(50.0); df['RSI_MA'] = df['RSI'].rolling(14, min_periods=1).mean()
        df['ADX'] = ta.adx(df['High'], df['Low'], df['Close'], length=14).iloc[:, 0].fillna(0.0); df['CHOP'] = ta.chop(df['High'], df['Low'], df['Close'], length=14).fillna(50.0)
        macd_df = ta.macd(df['Close'], fast=12, slow=26, signal=9); df['MACD'] = macd_df.iloc[:, 0].fillna(0); df['MACD_Sig'] = macd_df.iloc[:, 2].fillna(0)
        stoch_df = ta.stoch(df['High'], df['Low'], df['Close'], k=14, d=3, smooth_k=3); df['Stoch_K'] = stoch_df.iloc[:, 0].fillna(50); df['Stoch_D'] = stoch_df.iloc[:, 1].fillna(50)
        ap = (df['High'] + df['Low'] + df['Close']) / 3.0; esa = ap.ewm(span=10, adjust=False).mean(); d_wt = (ap - esa).abs().ewm(span=10, adjust=False).mean()
        df['WT1'] = ((ap - esa) / (0.015 * d_wt.replace(0, 1))).ewm(span=21, adjust=False).mean(); df['WT2'] = df['WT1'].rolling(4, min_periods=1).mean()
        df['Basis'] = df['Close'].rolling(20, min_periods=1).mean(); dev = df['Close'].rolling(20, min_periods=1).std(ddof=0).replace(0, 1) 
        df['BBU'] = df['Basis'] + (2.0 * dev); df['BBL'] = df['Basis'] - (2.0 * dev); df['BB_Width'] = (df['BBU'] - df['BBL']) / df['Basis'].replace(0, 1)
        df['BB_Width_Avg'] = df['BB_Width'].rolling(20, min_periods=1).mean(); df['BB_Delta'] = df['BB_Width'] - df['BB_Width'].shift(1).fillna(0); df['BB_Delta_Avg'] = df['BB_Delta'].rolling(10, min_periods=1).mean()
        kc_basis = df['Close'].rolling(20, min_periods=1).mean(); df['KC_Upper'] = kc_basis + (df['ATR'] * 1.5); df['KC_Lower'] = kc_basis - (df['ATR'] * 1.5)
        df['Squeeze_On'] = (df['BBU'] < df['KC_Upper']) & (df['BBL'] > df['KC_Lower']); df['Z_Score'] = (df['Close'] - df['Basis']) / dev.replace(0, 1)
        df['RSI_BB_Basis'] = df['RSI'].rolling(14, min_periods=1).mean(); df['RSI_BB_Dev'] = df['RSI'].rolling(14, min_periods=1).std(ddof=0).replace(0,1) * 2.0
        df['Vela_Verde'] = df['Close'] > df['Open']; df['Vela_Roja'] = df['Close'] < df['Open']; df['body_size'] = abs(df['Close'] - df['Open']).replace(0, 0.0001)
        df['upper_wick'] = df['High'] - df[['Open', 'Close']].max(axis=1); df['lower_wick'] = df[['Open', 'Close']].min(axis=1) - df['Low']
        ...
        df['PA_Pinbar_Buy'] = (df['lower_wick'] > df['body_size'] * 2.5) & (df['upper_wick'] < df['body_size']); df['PA_Pinbar_Sell'] = (df['upper_wick'] > df['body_size'] * 2.5) & (df['lower_wick'] < df['body_size'])
        ...
        df['RSI_Cross_Up'] = (df['RSI'] > df['RSI_MA']) & (df['RSI'].shift(1) <= df['RSI_MA'].shift(1)); df['RSI_Cross_Dn'] = (df['RSI'] < df['RSI_MA']) & (df['RSI'].shift(1) >= df['RSI_MA'].shift(1))
        df['Macro_Bull'] = df['Close'] >= df['EMA_200']; df['PP_Slope'] = (2*df['Close'] + df['Close'].shift(1) - df['Close'].shift(3) - 2*df['Close'].shift(4)) / 10.0
        target_start = pd.to_datetime(datetime.combine(start, datetime.min.time())) + timedelta(hours=offset); df = df[df.index >= target_start]
        df['Is_Train'] = True; gc.collect(); return df, "OK"
...
a_macd = df_global['MACD'].values; a_macd_sig = df_global['MACD_Sig'].values; a_stoch_k = df_global['Stoch_K'].values; a_stoch_d = df_global['Stoch_D'].values
a_chop = df_global['CHOP'].values; a_bbl = df_global['BBL'].values; a_bbu = df_global['BBU'].values; a_bw = df_global['BB_Width'].values
a_bwa_s1 = npshift(df_global['BB_Width_Avg'].values, 1, -1.0); a_wt1 = df_global['WT1'].values; a_wt2 = df_global['WT2'].values
a_ema50 = df_global['EMA_50'].values; a_ema200 = df_global['EMA_200'].values; a_atr = df_global['ATR'].values
a_rvol = df_global['RVol'].values; a_hvol = df_global['High_Vol'].values; a_vv = df_global['Vela_Verde'].values; a_vr = df_global['Vela_Roja'].values
a_rcu = df_global['RSI_Cross_Up'].values; a_rcd = df_global['RSI_Cross_Dn'].values; a_sqz_on = df_global['Squeeze_On'].values
a_bb_delta = df_global['BB_Delta'].values; a_bb_delta_avg = df_global['BB_Delta_Avg'].values; a_zscore = df_global['Z_Score'].values
a_rsi_bb_b = df_global['RSI_BB_Basis'].values; a_rsi_bb_d = df_global['RSI_BB_Dev'].values
...
a_wt1_s1 = npshift(a_wt1, 1, 0.0); a_wt2_s1 = npshift(a_wt2, 1, 0.0); a_macd_s1 = npshift(a_macd, 1, 0.0)
...
def calcular_señales_numpy(hitbox, therm_w, adx_th, whale_f):
    ...
    a_dsup = np.abs(a_c - a_tsup) / a_c * 100; a_dres = np.abs(a_c - a_tres) / a_c * 100; sr_val = a_atr * 2.0
    ...
    is_abyss = floor_w == 0; is_hard_wall = ceil_w >= therm_w; trinity_safe = a_mb & ~a_fk
    ...
    cond_lock_buy_bounce = is_grav_sup & (a_l <= a_tsup + tol) & (a_c > a_tsup) & a_vv; cond_lock_buy_break = is_grav_res & cross_up_res & a_hvol & a_vv
    cond_lock_sell_reject = is_grav_res & (a_h >= a_tres - tol) & (a_c < a_tres) & a_vr; cond_lock_sell_breakd = is_grav_sup & cross_dn_sup & a_vr
    ...
    rsi_vel = a_rsi - a_rsi_s1; pre_pump = ((a_h > a_bbu) | (rsi_vel > 5)) & flash_vol & a_vv; pump_memory = pre_pump | npshift_bool(pre_pump, 1) | npshift_bool(pre_pump, 2)
    ...
    s_dict['Q_Lock_Reject'] = cond_lock_sell_reject; s_dict['Q_Lock_Breakd'] = cond_lock_sell_breakd
    s_dict['Q_Neon_Dn'] = neon_dn; s_dict['Q_Defcon_Sell'] = cond_defcon_sell; s_dict['Q_Therm_Wall_Sell'] = cond_therm_sell_wall
    s_dict['Q_Therm_Panic_Sell'] = cond_therm_sell_panic; s_dict['Q_Nuclear_Sell'] = (a_rsi > 70) & (wt_overbought | wt_cross_dn); s_dict['Q_Early_Sell'] = (a_rsi > 70) & a_vr
    ...
    swing_range = a_ph30_l - a_pl30_l; fib_618_b = a_ph30_l - (swing_range * 0.618); fib_618_s = a_pl30_l + (swing_range * 0.618)
...
def optimizar_ia_tracker(s_id, cap_ini, com_pct, invest_pct, target_ado, dias_reales, buy_hold_money, epochs=1, cur_net=-float('inf'), cur_fit=-float('inf'), deep_info=None):
    ...
    best_dna = None
    if best_fit_live != -float('inf'): best_dna = vault.copy()

    iters = 250 * epochs; chunk_size = 250; chunks = max(1, iters // chunk_size)
    start_time = time.time(); n_len = len(a_c); split_idx = n_len; dias_entrenamiento = max(1, dias_reales)
    default_f = np.zeros(n_len, dtype=bool); ones_mask = np.ones(n_len, dtype=bool)

    for c in range(chunks):
        if st.session_state.get('abort_opt', False): 
            st.warning("🛑 OPTIMIZACIÓN ABORTADA. Extrayendo el campeón retenido en memoria..."); break
        ...
        for _ in range(chunk_size): 
            if best_dna is not None: 
                ...
                if rand_val < 0.50:
                    ...
                    r_w_rsi = round(best_dna.get('w_rsi', 0.0) + random.gauss(0, 0.1), 4); r_w_z = round(best_dna.get('w_z', 0.0) + random.gauss(0, 0.5), 4); r_w_adx = round(best_dna.get('w_adx', 0.0) + random.gauss(0, 0.1), 4)
                    r_th_b = round(best_dna.get('th_buy', 50.0) + random.gauss(0, 2.0), 2); r_th_s = round(best_dna.get('th_sell', -50.0) + random.gauss(0, 2.0), 2)
                    r_atr_tp = max(0.1, round(best_dna.get('atr_tp', 2.0) + random.gauss(0, 0.2), 2)); r_atr_sl = max(0.1, round(best_dna.get('atr_sl', 1.0) + random.gauss(0, 0.2), 2))
                    r_w_z = max(-10.0, min(10.0, r_w_z)); r_th_b = max(0.0, min(100.0, r_th_b)); r_th_s = max(-100.0, min(0.0, r_th_s))
                elif rand_val < 0.85:
                    ...
                    r_hitbox = best_dna.get('hitbox', 1.5); r_therm = best_dna.get('therm_w', 4.0); r_adx = best_dna.get('adx_th', 25.0); r_whale = best_dna.get('whale_f', 2.5)
                    r_w_rsi = round(best_dna.get('w_rsi', 0.0), 4); r_w_z = round(best_dna.get('w_z', 0.0), 4); r_w_adx = round(best_dna.get('w_adx', 0.0), 4)
                    r_th_b = round(best_dna.get('th_buy', 50.0), 2); r_th_s = round(best_dna.get('th_sell', -50.0), 2)
                    r_atr_tp = round(best_dna.get('atr_tp', 2.0), 2); r_atr_sl = round(best_dna.get('atr_sl', 1.0), 2)
                else:
                    ...
                    r_w_rsi = round(random.uniform(-2.0, 2.0), 4); r_w_z = round(random.uniform(-10.0, 10.0), 4); r_w_adx = round(random.uniform(-2.0, 2.0), 4)
                    r_th_b = round(random.uniform(0.0, 100.0), 2); r_th_s = round(random.uniform(-100.0, 0.0), 2)
            ...
            m_mask_dict = {"Bull Only": a_mb, "Bear Only": ~a_mb, "Organic_Vol": s_dict['Organic_Vol'], "Organic_Squeeze": s_dict['Organic_Squeeze'], "Organic_Safe": s_dict['Organic_Safe'], "Organic_Gaussian_Clean": s_dict['Organic_Gaussian_Clean'], "All-Weather": ones_mask, "Ignore": ones_mask}
            v_mask_dict = {"Trend": (a_adx >= r_adx), "Range": (a_adx < r_adx), "Organic_Pump": s_dict['Organic_Pump'], "Organic_Dump": s_dict['Organic_Dump'], "Organic_Gaussian_Clean": s_dict['Organic_Gaussian_Clean'], "All-Weather": ones_mask, "Ignore": ones_mask}
            m_mask = m_mask_dict.get(dna_macro, ones_mask); v_mask = v_mask_dict.get(dna_vol, ones_mask)
            ...
            net, pf, nt, mdd, wr = simular_crecimiento_exponencial_ia_core(
                a_h[:split_idx], a_l[:split_idx], a_c[:split_idx], a_o[:split_idx], a_atr[:split_idx], a_rsi[:split_idx], a_zscore[:split_idx], a_adx[:split_idx],
                f_buy_tactical[:split_idx], f_sell_tactical[:split_idx], r_w_rsi, r_w_z, r_w_adx, r_th_b, r_th_s,
                r_atr_tp, r_atr_sl, float(cap_ini), float(com_pct), float(invest_pct), 0.0, m_mask[:split_idx], v_mask[:split_idx]
            )

            fit_score = -float('inf') 
            if nt >= 3: 
                ado_actual = nt / max(1, dias_entrenamiento); ado_target_safe = max(0.1, target_ado)
                if net > 0:
                    safe_pf = min(pf, 3.0); dd_penalty = 1.0
                    if mdd > 25.0: dd_penalty = (25.0 / mdd)
                    if wr < 40.0: dd_penalty *= (wr / 40.0)
                    ado_factor = 1.0
                    if ado_actual < (ado_target_safe * 0.3): ado_factor = max(0.2, ado_actual / (ado_target_safe * 0.3))
                    fit_score = net * safe_pf * dd_penalty * ado_factor
                else:
                    ado_diff = abs(ado_actual - ado_target_safe) * 5; fit_score = net - mdd - ado_diff
            else: fit_score = net - 1000.0 

            if fit_score > best_fit_live:
                best_fit_live = fit_score; best_net_live = net; best_pf_live = pf; best_nt_live = nt
                bp = {'b_trigger': dna_b_trigger, 'b_confirm': dna_b_confirm, 'b_op': dna_b_op, 's_trigger': dna_s_trigger, 's_confirm': dna_s_confirm, 's_op': dna_s_op, 'macro': dna_macro, 'vol': dna_vol, 'hitbox': r_hitbox, 'therm_w': r_therm, 'adx_th': r_adx, 'whale_f': r_whale, 'fit': fit_score, 'net': net, 'winrate': wr, 'pf': pf, 'nt': nt, 'reinv': invest_pct, 'ado': ado_actual if nt >= 3 else 0.0, 'w_rsi': r_w_rsi, 'w_z': r_w_z, 'w_adx': r_w_adx, 'th_buy': r_th_b, 'th_sell': r_th_s, 'atr_tp': r_atr_tp, 'atr_sl': r_atr_sl}
                best_dna = bp.copy(); save_champion(s_id, bp); st.session_state[f'opt_status_{s_id}'] = True
            
        global_start = deep_info.get('start_time', start_time) if deep_info else start_time
        total_elapsed_sec = time.time() - global_start; h, rem = divmod(total_elapsed_sec, 3600); m, s = divmod(rem, 60); time_str = f"{int(h):02d}h:{int(m):02d}m:{int(s):02d}s"
        ...
        if deep_info:
            ...
            title = f"GENESIS LAB V220: {s_id}"; subtitle = f"Progreso: {pct_done}% | ADN Probados: {combos:,}<br>⏱️ Tiempo Ejecución: {time_str}"; color = "#00FFFF"
...
def run_backtest_eval(s_id, cap_ini, com_pct):
    ...
    f_tp = np.full(n_len, atr_tp); f_sl = np.full(n_len, atr_sl)
    f_buy = np.zeros(n_len, dtype=bool); f_sell = np.zeros(n_len, dtype=bool); default_f = np.zeros(n_len, dtype=bool); ones_mask = np.ones(n_len, dtype=bool)
    ...
    f_buy = f_buy_tactical | (score_arr > th_buy); f_buy &= (m_mask & v_mask)
