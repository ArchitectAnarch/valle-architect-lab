...
if 'gen_buy_rule' not in st.session_state: st.session_state.gen_buy_rule = 'Nuclear Buy'
if 'gen_sell_rule' not in st.session_state: st.session_state.gen_sell_rule = 'Defcon 2 Dn'
if 'gen_tp' not in st.session_state: st.session_state.gen_tp = 5.0
if 'gen_sl' not in st.session_state: st.session_state.gen_sl = 2.0
...
def cargar_y_preprocesar(exchange_id, sym, start, end, iv_down, iv_res, offset):
    try:
        ...
        if len(df) > 300:
            ...
            # --- CÁLCULO DE MÓDULOS GÉNESIS (V320) ---
            # Wavetrend
...
def generar_senales(df_sim, strat, w_factor, r_sens, macro_sh, atr_sh, def_buy, def_sell):
    ...
    if "TRINITY" in strat:
        ...
        # Asignación dinámica basada en la selección de UI
...
def renderizar_estrategia(strat_name, tab_obj, df_base):
    with tab_obj:
        ...
        # MÓDULO GÉNESIS (CONSTRUCTOR DE BOTS)
        if s_id == "GENESIS":
            ...
            st.session_state.gen_buy_rule = c_g1.selectbox("Gatillo de Compra", reglas_compra, index=reglas_compra.index(st.session_state.gen_buy_rule))
            st.session_state.gen_sell_rule = c_g2.selectbox("Gatillo de Cierre", reglas_venta, index=reglas_venta.index(st.session_state.gen_sell_rule))
            st.session_state.gen_tp = c_g3.slider("🎯 TP Base (%)", 0.5, 20.0, value=float(st.session_state.gen_tp), step=0.5)
            st.session_state.gen_sl = c_g4.slider("🛑 SL (%)", 0.5, 15.0, value=float(st.session_state.gen_sl), step=0.5)
            
            if st.button("🧪 Ejecutar Algoritmo Mutante"):
                st.rerun()
                
            if st.button("🚀 Que la IA construya el Bot Perfecto (Fuerza Bruta)", type="primary"):
                ph_holograma.markdown(css_spinner, unsafe_allow_html=True)
                best_fit = -999
                bp = {}
                # Cruza todas las compras con todas las ventas
                ...
                if bp:
                    ...
                    st.session_state.gen_tp = bp['tp']
                    st.session_state.gen_sl = bp['sl']
            ...
            eq_curve, divs, cap_act, t_log, pos_ab = ejecutar_simulacion(df_strat, strat_name, st.session_state.gen_tp, st.session_state.gen_sl, capital_inicial, 0.0, comision_pct)
            
        else:
            # BLOQUE ESTRATEGIAS NORMALES (TRINITY/JUGG/DEFCON)
            with st.form(f"form_{s_id}"):
                ...
                if s_id == "TRINITY":
                    ...
                    t_radar = st.slider("📡 Tolerancia Target Lock (%)", 0.1, 5.0, value=float(st.session_state[f'radar_{s_id}']), step=0.1)
        ...
        # --- BLOQUE COMÚN DE GRÁFICAS PARA TODAS LAS PESTAÑAS ---
